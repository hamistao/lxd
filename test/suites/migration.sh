...
migration() {
  ...
  if [ "${lxd_backend}" = "btrfs" ] || [ "${lxd_backend}" = "dir" ] || [ "${lxd_backend}" = "zfs" ]; then
    ...
    [ -z "$(lxc_remote storage volume get "${pool}" container/l2:udssr volatile.rootfs.size)" ]
  else
    # Block based
    [ "$(lxc_remote info l2:udssr | awk '/Disk total:/ {found=1} found && /root:/ {print $2; exit}')" = "10.00GiB" ]
    [ "$(lxc_remote storage volume get "${pool}" container/l2:udssr volatile.rootfs.size)" = "10GiB" ]
  fi
